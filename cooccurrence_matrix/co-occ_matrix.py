...
#         # Fallback to local if not found (though user said they exist)
...
#         # Recalculate if absolutely necessary (should not happen based on prompt)
...
#         # 1. Try loading from OLD results (results1)
...
#         # 2. Try loading from NEW results (if we ran this script twice)
#         save_path = os.path.join(MATRICES_DIR, f"ppmi_w{window_size}.npz")
#         if os.path.exists(save_path):
#              return sp.load_npz(save_path)

#         # 3. Calculate from scratch (Only if missing in both)
...
#         # Save SVD results to the NEW folder (results2)
...
#     # We shouldn't need corpus if matrices exist in results1
#     corpus = [] 
...
class MatrixBuilder:
    ...
    def load_vocab(self, path, top_k=25000):
        ...
        # 1. Read Raw Counts (No merging, no lowercasing, preserve case & numbers)
        raw_counts = []
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                parts = line.strip().split('\t')
    ...
    def preprocess(self, text):
        # User Instruction: "just allow hyphen, other things in the middle... dont convert to lower case"
        # We split by whitespace, then strip ONLY leading/trailing punctuation that are NOT part of the word.
        # This preserves "1.8", "co-operate", "Anne's" but removes "word," or "(word)".
    ...
    def build_cooccurrence(self, sentences, window_size):
        ...
        for idx, sent in enumerate(sentences):
            ...
            # Preprocess text (Case Sensitive now!)
            tokens = [t for t in self.preprocess(text) if t in self.vocab]
...
class MatrixTransformer:
    ...
    def determine_optimal_d_and_embed(matrix, window_size, fixed_d=300):
        ...
        # Ensure we don't ask for more components than the matrix has
        k = min(fixed_d, matrix.shape[1] - 1)
        
        print(f"[Process] Calculating SVD (fixed d={k})...")
        svd = TruncatedSVD(n_components=k, n_iter=5, random_state=42)
        
        # ONE pass only: fit AND transform
